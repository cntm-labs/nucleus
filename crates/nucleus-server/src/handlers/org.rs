...
fn parse_project_id(claims: &nucleus_auth::jwt::NucleusClaims) -> Result<ProjectId, AppError> {
    claims
        .aud
        .parse()
...
fn parse_user_id(claims: &nucleus_auth::jwt::NucleusClaims) -> Result<UserId, AppError> {
    claims
        .sub
        .parse()
        .map_err(|_| AppError::Auth(nucleus_core::error::AuthError::TokenInvalid))
...
pub async fn handle_create_invitation(
    ...
    let generated = nucleus_org::invitation::InvitationService::generate();
    let role = req.role.unwrap_or_else(|| "member".to_string());
    let base_url = "https://nucleus.dev".to_string();
    let invite_url = nucleus_org::invitation::InvitationService::build_url(
        &base_url,
        &slug,
    ...
    Ok(Json(serde_json::json!({
        "id": nucleus_core::types::InvitationId::new().to_string(),
        "email": req.email,
        "role": role,
        "status": "pending",
