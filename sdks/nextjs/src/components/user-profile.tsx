  ...
  return (
    <div style={s.card}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button onClick={() => setSection('profile')} style={{ ...s.secondaryButton, flex: 1, background: section === 'profile' ? '#e5e7eb' : undefined }}>{t('userProfile.profileTab')}</button>
        <button onClick={() => setSection('password')} style={{ ...s.secondaryButton, flex: 1, background: section === 'password' ? '#e5e7eb' : undefined }}>{t('userProfile.passwordTab')}</button>
      </div>
      {section === 'profile' && (
        <form onSubmit={handleProfileUpdate}>
          ...
          <button type="submit" disabled={isLoading} style={{ ...s.button, opacity: isLoading ? 0.7 : 1 }}>{isLoading ? t('userProfile.saving') : t('userProfile.save')}</button>
        </form>
      )}
      {section === 'password' && (
        <form onSubmit={handlePasswordUpdate}>
          <input type="password" placeholder={t('userProfile.currentPassword')} value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} style={s.input} required />
          <input type="password" placeholder={t('userProfile.newPassword')} value={newPassword} onChange={e => setNewPassword(e.target.value)} style={{ ...s.input, marginBottom: 16 }} required />
          <button type="submit" disabled={isLoading} style={{ ...s.button, opacity: isLoading ? 0.7 : 1 }}>{isLoading ? t('userProfile.updating') : t('userProfile.updatePassword')}</button>
        </form>
      )}
    </div>
  )
