...
export function OrgProfile({ onInvite }: OrgProfileProps) {
  ...
  if (!organization) return <div style={s.card}><p style={{ color: '#6b7280' }}>No organization selected</p></div>
  ...
  return (
    <div style={s.card}>
      ...
      <div style={{ marginBottom: 16 }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Members</h3>
        {members.map(member => (
          <div key={member.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f3f4f6' }}>
            ...
            }} style={{ border: 'none', background: 'none', color: '#dc2626', cursor: 'pointer', fontSize: 13 }}>Remove</button>
          </div>
        ))}
      </div>
      <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Invite Member</h3>
      <form onSubmit={handleInvite}>
        <input type="email" placeholder="Email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} style={s.input} required />
        <select value={inviteRole} onChange={e => setInviteRole(e.target.value)} style={{ ...s.input, marginBottom: 16 }}>
          <option value="member">Member</option>
          <option value="admin">Admin</option>
        </select>
        <button type="submit" disabled={isLoading} style={{ ...s.button, opacity: isLoading ? 0.7 : 1 }}>{isLoading ? 'Inviting...' : 'Send Invitation'}</button>
      </form>
    </div>
  )
}
