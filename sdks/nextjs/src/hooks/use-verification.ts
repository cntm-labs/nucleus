...
export function useVerification() {
  ...
  const sendEmailVerification = useCallback(async (email: string) => {
    ...
    try { return await _api.sendEmailVerification(getToken()!, email) }
  ...
  const confirmEmailVerification = useCallback(async (code: string) => {
    setIsLoading(true); setError(null)
    try { return await _api.confirmEmailVerification(getToken()!, code) }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
    finally { setIsLoading(false) }
  }, [_api, getToken])
  ...
  const sendPhoneVerification = useCallback(async (phone: string) => {
    setIsLoading(true); setError(null)
    try { return await _api.sendPhoneVerification(getToken()!, phone) }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
    finally { setIsLoading(false) }
  }, [_api, getToken])
  ...
  const confirmPhoneVerification = useCallback(async (code: string) => {
    setIsLoading(true); setError(null)
    try { return await _api.confirmPhoneVerification(getToken()!, code) }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
    finally { setIsLoading(false) }
  }, [_api, getToken])
