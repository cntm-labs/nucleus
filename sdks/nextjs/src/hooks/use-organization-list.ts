...
export function useOrganizationList() {
  ...
  const fetchOrganizations = useCallback(async () => {
    ...
    try { const list = await _api.getOrganizations(getToken()!); setOrganizations(list); return list }
  ...
  const createOrganization = useCallback(async (name: string, slug: string) => {
    setError(null)
    try { const org = await _api.createOrganization(getToken()!, name, slug); setOrganizations(prev => [...prev, org]); return org }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
  }, [_api, getToken])

  const setActiveOrganization = useCallback((org: NucleusOrganization | null) => { _setOrganization(org) }, [_setOrganization])
