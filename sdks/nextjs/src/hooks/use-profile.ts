...
export function useProfile() {
  ...
  const updateProfile = useCallback(async (data: { first_name?: string; last_name?: string; avatar_url?: string }) => {
    ...
    try { const user = await _api.updateUser(getToken()!, data); _setUser(user); return user }
  ...
  const updatePassword = useCallback(async (currentPassword: string, newPassword: string) => {
    setIsLoading(true); setError(null)
    try { return await _api.updatePassword(getToken()!, currentPassword, newPassword) }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
    finally { setIsLoading(false) }
  }, [_api, getToken])
  ...
  const updateEmail = useCallback(async (email: string) => {
    setIsLoading(true); setError(null)
    try { return await _api.updateEmail(getToken()!, email) }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed'); throw err }
    finally { setIsLoading(false) }
  }, [_api, getToken])
