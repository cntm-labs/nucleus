...
export function OrgProfile({ onInvite }: OrgProfileProps) {
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      const list = await _api.getMembers(getToken()!, organization.id)
      setMembers(list)
    } catch { /* ignore */ }
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      await _api.createInvitation(getToken()!, organization.id, inviteEmail, inviteRole)
      setInviteEmail('')
      onInvite?.()
    } catch (err) {
  ...
  return (
    <div style={s.card}>
      ...
      <p style={{ fontSize: 13, color: '#6b7280', textAlign: 'center', marginBottom: 16 }}>{organization.slug}</p>
