...
use crate::identity::handlers::me::{UpdateProfileRequest, UserProfileResponse};
...
pub async fn handle_get_me(
    ...
    let resp =
        crate::identity::handlers::me::get_me(&state.user_service, &project_id, &user_id).await?;
    Ok(Json(resp))
}
...
pub async fn handle_update_me(
    ...
    let (project_id, user_id) = auth_from_jwt(&jwt);
    let resp =
        crate::identity::handlers::me::update_me(&state.user_service, &project_id, &user_id, req)
            .await?;
    Ok(Json(resp))
}
...
pub async fn handle_delete_me(
    ...
    let (project_id, user_id) = auth_from_jwt(&jwt);
    crate::identity::handlers::me::delete_me(&state.user_service, &project_id, &user_id).await?;
